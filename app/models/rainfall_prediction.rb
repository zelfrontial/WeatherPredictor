class RainfallPrediction
	def calculatePrediction timeArray, valueArray
		...
		if bestfit == linear || bestfit == polynomial
			...
			(0..18).each do |x|
				r = Rainfall.new
				r.rainfall = bestfit.coefficients[0] + (bestfit.coefficients[1] * Math.log((timeArray.last + (x*10))))
				rainfall_prediction.push(r)
			end
        end
			...
			(0..18).each do |x|
				r = Rainfall.new
				r.rainfall = bestfit.coefficients[0] * (Math::E**(bestfit.coefficients[1] * (timeArray.last + (x*10))))
				rainfall_prediction.push(r)
			end
        end
        [rainfall_prediction, bestfit.mse]
	end
end
